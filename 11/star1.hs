...
process_item :: Monkey -> Int -> [Monkey] -> [Monkey]
process_item Monkey { op = expr, check = divisor, left_dest = l, right_dest = r } value monkeys = updated_monkeys
  where new_value = (eval expr value) `div` 3
        dest = if new_value `mod` divisor == 0 then r else l
        updated_monkeys = throw_item dest monkeys new_value


process_monkey :: [Monkey] -> Int -> [Monkey]
process_monkey monkeys i = (update updated_monkeys i (\_ -> new_monkey))
  where m = monkeys!!i
        count = inspect_count m
        new_monkey = m { items = [], inspect_count = count + (length $ items m) }
        updated_monkeys = foldl (flip (process_item m)) monkeys (items m)

loop :: (a -> a) -> Int -> a -> a
loop thunk 0 initial = initial
loop thunk n initial = loop thunk (n - 1) (thunk initial)

perform_round :: [Monkey] -> [Monkey]
perform_round ms = foldl process_monkey ms [0..((length ms) - 1)]

calc_monkey_business :: [Monkey] -> Int
calc_monkey_business = product . (take 2) . reverse . sort . map inspect_count

solution :: String -> String
solution input = (show $ calc_monkey_business $ loop perform_round  20 $ parse_monkeys input) ++ "\n"
