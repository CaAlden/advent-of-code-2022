...
solution input = show sand_count ++ "\n"
  where ls = parse_input_lines input
        ...
        rows = (snd $ snd bounds) + 1
        columns = (right - left) + 1
        source = (0 - top, 500 - left)
        fixed_lines = map (\((x, y), (x2, y2)) -> ((y - top, x - left), (y2 - top, x2 - left))) ls
        empty_grid = replace source Source $ generateGrid (rows, columns) Air
        initial_grid = foldr (draw_line Rock) empty_grid fixed_lines
