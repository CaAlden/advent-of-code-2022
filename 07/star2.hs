...
solution i = show $
  minimum $
  map (\x -> (calculate_size x, name x)) $
  filter (\x -> is_dir x && (calculate_size x) >= needed_space) files
    where file_tree = parse_directory_structure i
          files = fold_file_tree (\acc ft -> ft:acc) file_tree []
          free_space = 70000000 - (calculate_size file_tree)
